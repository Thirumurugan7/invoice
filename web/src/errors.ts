...
const EXPLAIN: Record<string, (a: readonly unknown[]) => string> = {
  ...
  CollateralRequired: (a) => `To accept, debtor ${short(a[0])} must hold ${yen(a[1] as bigint)} JPYC of collateral (has ${yen(a[2] as bigint)}). Lock the difference on the Debtor tab first.`,
  NameTaken: (a) => `That exact name is already used by ${short(a[0])}. Names are unique; if it is yours, ask the operator to take down the other one.`,
  BadGrade: () => 'Grade must be 1–5 and the requirement at most 10000 bps (100%).',
  BelowRequired: (a) => `Can't withdraw that much: ${yen(a[0] as bigint)} backs your accepted, unpaid invoices and stays locked until they are paid; only ${yen(a[1] as bigint)} would remain.`,
