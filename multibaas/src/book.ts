...
/// Only accepted invoices are claims; like the registry's outstandingOf, pending ones don't count yet.
export const acceptedQuery: MultiBaas.EventQuery = {
  events: [{ eventName: 'InvoiceAccepted', select: [{ type: 'input', inputIndex: 0, alias: 'id' }], filter: byAlias(LABELS.registry) }],
};

/// Defaulted invoices are written off on-chain (outstanding goes to 0; holders redeem pro-rata).
export const defaultedQuery: MultiBaas.EventQuery = {
  events: [{ eventName: 'InvoiceDefaulted', select: [{ type: 'input', inputIndex: 0, alias: 'id' }], filter: byAlias(LABELS.registry) }],
};
...
if (import.meta.url === `file://${process.argv[1]}`) {
  const [invoices, paid, trades, rejected, credit, accepted, defaulted] = await Promise.all([
    run(registeredQuery),
    run(paidQuery),
    run(tradesQuery, 20),
    run(rejectedQuery),
  ...
  const acceptedIds = new Set(accepted.map((r) => String(r.id)));
  const defaultedIds = new Set(defaulted.map((r) => String(r.id)));
  ...
  for (const r of invoices) {
    ...
    const state = isRejected ? 'rejected' : defaultedIds.has(id) ? 'defaulted' : !acceptedIds.has(id) ? 'pending acceptance' : undefined;
    // Matches the registry's outstandingOf: accepted face minus payments; nothing while pending, rejected or defaulted.
    const outstanding = state ? 0n : BigInt(String(r.face)) - BigInt(paidById.get(id) ?? '0');
    ...
    console.log(`#${r.id} ${refById.get(String(r.id)) ?? ''}  face ${yen(String(r.face))}  due ${due}  rate@issue ${Number(r.rate_at_issue_bps) / 100}%  outstanding ${yen(outstanding.toString())}${state ? `  (${state})` : ''}`);
  }
